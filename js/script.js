...
function createTodoItem(title) {
    const checkbox = createElement("input", { type: "checkbox", className: "checkbox" }),
        ...
        editButton = createElement("button", { className: "edit" }, "Редактировать"),
...
function bindEvents(todoItem) {
    const checkbox = todoItem.querySelector(".checkbox"),
        ...
        delBtn = todoMain.querySelector(".delete");
...
function addTodoItem(event) {
    ...
    if (addInput.value.trim() !== "" && addInput.value.length <= 38) {
        const listItem = createTodoItem(addInput.value);
        todoList.appendChild(listItem);

        todoForm.reset();
    } else if (addInput.value.trim() !== "" && addInput.value.length >= 38) {
        const listItem = createTodoItem(`${addInput.value.substring(0, 38)}...`);
        todoList.appendChild(listItem);
